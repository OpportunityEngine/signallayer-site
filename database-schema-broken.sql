-- Revenue Radar Database Schema
-- SQLite schema for production data storage
-- This replaces JSON file storage with proper relational database
...
-- Ingestion runs (existing invoice upload sessions)
...
-- Invoice line items extracted from runs
...
-- MLA (Master Lease Agreement) records
...
-- MLA review activity (tracks when reps review MLAs for SPIF)
...
-- Sales opportunities detected by system
...
-- SPIF (Sales Performance Incentive Fund) configurations
...
-- SPIF leaderboard/results (real-time standings)
...
-- Telemetry events from browser extension
...
-- Leads found by system (contacts for accounts)
...
-- Active SPIFs with current top 3 leaders
