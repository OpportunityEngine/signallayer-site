cat > parsers/registry/columnInvoiceLoose.js <<'JS'
function match(input) {
  ...
  if (/\b(Qty|Quantity)\b/i.test(t) && /\b(Unit|Unit Price|Price)\b/i.test(t)) {
    score += 0.45; reasons.push("qty_and_unit_headers");
  }
...
function parse(input) {
  ...
  // Heuristic: capture rows that look like: DESC ... QTY ... $UNIT ... $TOTAL
  // Works even if spacing is inconsistent.
  for (const raw of lines) {
    const line = (raw || "").replace(/\s+/g, " ").trim();
    if (!line) continue;

    // Skip obvious totals/footer
    if (/^(Subtotal|Tax|Total|Balance|Payment|Amount Due)\b/i.test(line)) continue;

    // Try patterns in order
    // 1) DESC  QTY  $UNIT  $TOTAL
    ...
    // 2) SKU DESC  QTY  $UNIT  $TOTAL  (SKU is token at start)
